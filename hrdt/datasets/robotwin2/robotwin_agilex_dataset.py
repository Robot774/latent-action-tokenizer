    ...
    def load_language_embedding(self, hdf5_file_path):
        ...
        try:
            ...
            if not os.path.exists(embedding_path):
                # print(f"Warning: Task embedding file not found: {embedding_path}, using default embedding")
                # # Return default T5 embedding (typical T5 embedding dimensions: sequence_length=10, hidden_size=512)
                default_embedding = torch.zeros(1, 4096, dtype=torch.float32)
            ...
            if embeddings is None:
                print(f"Warning: No embeddings found in {embedding_path}, using default embedding")
                default_embedding = torch.zeros(1, 4096, dtype=torch.float32)
                return default_embedding
        ...
        try:
            with h5py.File(hdf5_file, 'r', swmr=True, libver='latest') as f:
                ...
                try:
                    ...
                    # Calculate future frame index (end of action sequence)
                    future_index = min(action_end, max_index + 1)
                    ...
                    for cam_idx, cam_name in enumerate(self.cameras):
                        ...
                        if cam_path and cam_path in f:
                            ...
                            # Current images: 复用现有接口
                            current_img_frames = self.parse_img_data(camera_data, index)
                            current_images.append(current_img_frames)
                            
                            # Future images: 复用现有接口，仅调整idx
                            future_img_frames = self.parse_img_data(camera_data, future_index)
                            future_images.append(future_img_frames)
                        else:
                            print(f"Warning: Camera {cam_name} not found in {hdf5_file}")
                            return None
                    
                    # Ensure we have the correct number of cameras
                    if len(current_images) != self.num_cameras or len(future_images) != self.num_cameras:
                        print(f"Error: Expected {self.num_cameras} cameras, but got {len(current_images)} current and {len(future_images)} future")
                        return None
                    
                    # Convert to numpy arrays with shape [num_cameras, history_size, H, W, 3]
                    current_img_frames_np = np.array(current_images)
                    future_img_frames_np = np.array(future_images)
                ...
                result = {
                    "current_images": current_img_frames_np,  # Current frame images (history frames)
                    "current_images_mask": current_images_mask,  # Current image masks
                    "future_images": future_img_frames_np,    # Future frame images (end of action sequence)
                    "future_images_mask": future_images_mask,  # Future image masks
                    ...
                    # Add default camera parameters for collate compatibility with EgoDex
                    "current_camera_extrinsics": np.eye(4)[None].astype(np.float32),  # (1, 4, 4) default identity
                    "action_camera_extrinsics": np.eye(4)[None].repeat(self.chunk_size, axis=0).astype(np.float32),  # (chunk_size, 4, 4)
                    "camera_intrinsics": np.array([[500.0, 0.0, 320.0], [0.0, 500.0, 240.0], [0.0, 0.0, 1.0]], dtype=np.float32),  # (3, 3) default
                }

    ...
    for item in os.listdir(TARGET_DIR):
        ...
        if os.path.isdir(item_path):
            if item in ['extra', 'test'] or item.startswith('part'):
                # Check if this is a direct task directory or has nested structure
                subdirs = os.listdir(item_path)
                # If there's a subdirectory with the same name, use that (nested structure)
                if item in subdirs:
                    nested_path = os.path.join(item_path, item)
                    if os.path.isdir(nested_path):
                        dataset_dirs.append(nested_path)
                    else:
    ...
    try:
        ...
        for file_info in file_list:
            try:
                ...
                import traceback
                error_msg = f"Process {process_id}: Error: Processing file {file_info['hdf5_path']} encountered problem: {str(e)}\nTraceback:\n{traceback.format_exc()}"
                print(error_msg)
                failed_count += 1
                progress_queue.put(('failed', process_id))
                continue
        ...
        import traceback
        error_msg = f"Process {process_id} encountered serious error: {str(e)}\nTraceback:\n{traceback.format_exc()}"
        print(error_msg)
        progress_queue.put(('error', process_id, error_msg))
